...
export async function scrapeAltexProduct(url: string) {
  if (!url) return;
  ...
  try {
    ...
    const data = await page.evaluate(async (url) => {
      ...
      const currentPrice = document.querySelector('.Price-int.leading-none')?.textContent || '0'
			...
			const processedCurrentPrice = processAltexPrice();
...
export async function scrapeAltexPrice(url: string) {
  if (!url) return;
